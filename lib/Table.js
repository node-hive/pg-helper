...
_.extend(Table.prototype, {
    ...
    insert: function (client, record, returning, cb) {
        ...
        if (cb) {
            client.query(this.insert_sql(record, returning), cb);
        } else {
            var deferred = Q.defer();

          if (_DEBUG)  console.log('insert SQL: %s', this.insert_sql(record, returning));
    ...
    insert_sql: function (record, returning) {
        ...
        return _ISQL({table: this, _q: _q, _: _, returning: returning, fields: fields, values: values})
    }
